...
void init_textures(SDL_Renderer *renderer, resources_t *textures, world_t *world) {
    ...
    textures->finish_line = charger_image_transparente("IMG/finish_line.bmp", renderer);
}
...
void refresh_graphics(SDL_Renderer *renderer, world_t *world, resources_t *textures) {
    ...
    apply_sprite(renderer, textures->finish_line, &world->ligne_arivee);

    char timer[20];
    sprintf(timer, "Temps : %.3lf", (double)(SDL_GetTicks())/(double)1000);
    apply_text(renderer, 0, 0, 200, 100, timer, textures->font,(SDL_Color){243, 89, 0});
